// 로컬 데이터 저장소 (localStorage 활용)
const STORAGE_KEY = 'songpa_tech_posts'
...
export const postService = {
  ...
  async getAllPosts() {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(getPostsFromStorage())
      }, 100) // 약간의 지연으로 실제 API 호출처럼 보이게
    })
  },
  ...
  async getPostById(id) {
    try {
      ...
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          const post = getPostsFromStorage().find(p => p.id === parseInt(id))
          if (post) {
            resolve(post)
          } else {
  ...
  async getPostsByCategory(category) {
    return new Promise((resolve) => {
      setTimeout(() => {
        const filteredPosts = getPostsFromStorage().filter(post => 
  ...
  async getCategories() {
    return new Promise((resolve) => {
      setTimeout(() => {
        const categories = [...new Set(getPostsFromStorage().map(post => post.category))]
        resolve(categories)
      }, 100)
    })
  },
  ...
  async getCompanies() {
    return new Promise((resolve) => {
      setTimeout(() => {
        const companies = [...new Set(getPostsFromStorage().map(post => post.company))]
        resolve(companies)
      }, 100)
    })
  }
